...
class Extract(Task):
    ...
    def _run(self, data: Dataset = None) -> Dataset:
        ...
        if (
            ...
            if os.path.exists(self._filepath_download):
                ...
                if "y" in x.lower():
                    self._download(
                        source=self._source, destination=self._filepath_download
                    )
    ...
    def _process_response(
        ...
        self._teardown_process_response(
            destination=destination, i=i, downloaded=downloaded
        )
    ...
    def _teardown_process_response(
        self, destination: str, i: int, downloaded: int
    ) -> dict:
    ...
    def _decompress(self, source: str, destination: str) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdirname:
            ...
            tempfilepath = os.path.join(
                tempdirname, self._datasource_config.filepath_extract
            )

            self._summary["Size Extracted (Mb)"] = round(
        ...
        self._logger.info(
            "\tDecompression Complete! {} Mb Extracted.".format(
                str(self._summary["Size Extracted (Mb)"])
            )
        )
        self._logger.debug(
            "\t\t\tCompleted {} {}".format(
...
class TransformETL(Task):
    ...
    def summary(self) -> dict:
        ...
        missing = self._before.to_frame(name="Before").join(
            self._after.to_frame(name="After")
        )

        subtitle = "Missing Values Replacement: Dataset {} / {} Stage".format(
            self._config.name, self._config.stage
        )
        ...
        d = {
            ...
            "Pct": {
                "Before": round(before / cells * 100, 2),
                "After": round(after / cells * 100, 2),
            },
        }
