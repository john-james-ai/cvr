...
# Modified : Thursday, February 3rd 2022, 10:36:01 am                          #
...
class AssetRepo(ABC):
    ...
    def get_assets(self) -> pd.DataFrame:
        ...
        return self._io.load(self._registry_filepath)

    # ------------------------------------------------------------------------ #
    def delete_asset(
        self, asset_type: str, stage: str, name: str, ignore_errors=True
    ) -> None:
        ...
        versions_to_delete = self._search_registry_by_asset(
            asset_type=asset_type, name=name, stage=stage
        )
        # Remove asset versions by filepath
        for filepath in versions_to_delete["filepath"]:
            self._io.remove(filepath)

        # Remove registrations by aid
        for registration in versions_to_delete["aid"]:
            registry = registry.loc[registry["aid"] != registration]
